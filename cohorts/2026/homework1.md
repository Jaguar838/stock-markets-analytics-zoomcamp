...
### Вопрос 1: Новые компании в S&P 500
**Суть:** Нужно спарсить таблицу с Wikipedia и найти, в какой год (начиная с 2020-го) в индекс добавили больше всего новых компаний. 
*Дополнительно:* посчитать, сколько текущих компаний находятся в индексе более 20 лет.
**Вопрос к размышлению:** Как ты преобразуешь столбец с датами в pandas, чтобы извлечь только год и сгруппировать по нему данные для подсчета количества записей?

### Вопрос 2: Доходность индексов с начала года (YTD)
**Суть:** Сравнить доходность 10 мировых индексов (Китай, Германия, Япония и др.) с американским S&P 500 за период с 1 января по 21 августа 2026 года. 
*Дополнительно:* повторить сравнение за 3, 5 и 10 лет.
**Вопрос к размышлению:** Какую формулу ты применишь к ценам закрытия (`Close`), чтобы рассчитать процентный рост за период: `(Цена_конца / Цена_начала) - 1` или другую?

### Вопрос 3: Анализ рыночных коррекций (просадок)
**Суть:** Найти медианную глубину и длительность падений S&P 500 с 1950 года. Коррекция = падение на **5% и более** от последнего исторического максимума.
**Вопрос к размышлению:** Как с помощью pandas отследить "бегущий максимум" (running maximum) цены и найти минимальную цену до момента, когда этот максимум будет обновлен? (Подсказка: обрати внимание на метод `.cummax()`).

### Вопрос 4: Реакция Amazon (AMZN) на отчеты о прибыли
**Суть:** Узнать, как растет цена акций Amazon в течение 2 дней после публикации отчета, если прибыль оказалась *выше* ожиданий аналитиков (positive earnings surprise). Нужно найти медиану этого роста и его корреляцию с величиной превышения ожиданий.
**Вопрос к размышлению:** Как ты объединишь (merge/join) таблицу с датами отчетов и таблицу с историческими ценами, чтобы точно взять цену закрытия в день отчета и через два торговых дня после него?

### Вопросы 5 и 6 (Опционально): Идеи и метрики
**Суть:** Придумать тему для финального проекта (например, предсказание цен) и найти 1-2 новых источника данных или метрики для него (например, объемы торгов, новостной фон, макроэкономические индикаторы).
**Вопрос к размышлению:** Какие данные, помимо исторических цен, ты как архитектор мог бы добавить в систему, чтобы повысить точность прогноза?

---
Есть непонятный финансовый термин?
